...
const ContentPreview = ({
    ...
    switch (contentType) {
      case 'gamma':
        ...
        return <div className="relative w-full h-full">
            {isLoading && <div className="absolute inset-0 flex items-center justify-center bg-foreground/5">
                <Loader2 className="w-8 h-8 text-cyan-accent animate-spin" />
              </div>}
            <iframe src={gammaEmbedUrl} className={cn('w-full h-full border-none transition-opacity duration-300', isLoading ? 'opacity-0' : 'opacity-100')} title={post.title} loading="lazy" onLoad={handleIframeLoad} allow="fullscreen" />
          </div>;
      case 'video':
        ...
        return <div className="relative w-full h-full">
            ...
            {embedUrl ? <iframe src={embedUrl} className={cn('w-full h-full border-none transition-opacity duration-300', isLoading ? 'opacity-0' : 'opacity-100')} title={post.title} loading="lazy" onLoad={handleIframeLoad} allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen /> : <div className="flex flex-col items-center justify-center h-full text-text-light/50 gap-4">
      ...
      case 'image':
        ...
        return <div className="w-full h-full overflow-auto p-4 sm:p-6 flex flex-col">
            {images.length === 1 ? <img src={images[0]} alt={post.title} className="w-full max-h-[60%] object-contain rounded-lg" /> : <div className="grid grid-cols-2 gap-4">
      ...
      case 'audio':
        return <div className="flex flex-col items-center justify-center h-full gap-6 p-8">
            ...
            {post.embedUrl && <audio controls className="w-full max-w-md">
      ...
      case 'pdf':
        return <div className="relative w-full h-full">
            ...
            <iframe src={post.pdfUrl} className={cn('w-full h-full border-none transition-opacity duration-300', isLoading ? 'opacity-0' : 'opacity-100')} title={post.title} loading="lazy" onLoad={handleIframeLoad} />
          </div>;
      case 'article':
        return <div className="w-full h-full overflow-auto p-6 sm:p-8">
      ...
      case 'link':
        return <div className="flex flex-col items-center justify-center h-full gap-6 p-8">
            ...
            <div className="text-center">
              ...
              <a href={post.link} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-6 py-3 bg-cyan-accent text-dark-bg font-medium rounded-lg hover:bg-cyan-accent/90 transition-colors">
      ...
      default:
        return <div className="flex flex-col items-center justify-center h-full text-text-light/50 gap-4">
  ...
  return <div className={cn('w-full h-full bg-foreground/5 rounded-xl overflow-hidden flex flex-col', className)}>
      {/* Header */}
      <div className={cn('shrink-0 border-b border-text-light/10', showFullMetadata ? 'p-4' : 'p-3')}>
        {showFullMetadata ?
      ...
      <div className="space-y-3">
            ...
            <div className="flex items-start justify-between gap-4">
              ...
              <div className="flex items-center gap-1 shrink-0">
                <button onClick={() => shareToLinkedIn(shareUrl, post.title)} className="p-2 text-text-light/50 hover:text-[#0077b5] hover:bg-[#0077b5]/10 rounded-lg transition-all duration-200" title="Share on LinkedIn">
                  <Linkedin size={16} />
                </button>
                <button onClick={() => copyToClipboard(postUrl)} className="p-2 text-text-light/50 hover:text-cyan-accent hover:bg-cyan-accent/10 rounded-lg transition-all duration-200" title="Copy link">
                  <Link2 size={16} />
                </button>
              </div>
            </div>
            ...
            {/* Tags */}
            {post.tags && post.tags.length > 0 && <div className="flex items-center gap-2 flex-wrap">
                <Tag size={12} className="text-text-light/30" />
                {post.tags.map(tag => <span key={tag} className="px-2 py-0.5 text-[10px] font-medium rounded bg-text-light/5 text-text-light/50">
                    {tag}
                  </span>)}
              </div>}
          </div> :
