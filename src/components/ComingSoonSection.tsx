  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-8">
        {/* Section Header */}
        <div className="mb-20">
          <p className="text-sm tracking-[0.3em] text-cyan-accent uppercase mb-6">What's Next</p>
        ...
        <div className="grid lg:grid-cols-2 gap-10">
          {upcomingProjects.map((project, index) => (
            <div
              key={index}
              className="relative p-10 rounded-2xl bg-foreground/5 border border-foreground/10 overflow-hidden"
            >
              {/* Status Badge */}
              <div className="absolute top-6 right-6">
                <span className={`inline-flex items-center gap-2 px-4 py-2 text-sm tracking-wider uppercase rounded-full ${
                  ...
                  {project.status === 'In Development' ? <Clock className="w-4 h-4" /> : <Sparkles className="w-4 h-4" />}
                  {project.status}
                </span>
              </div>

              <div className="flex items-start gap-6 mb-8">
                <div className="w-20 h-20 rounded-xl overflow-hidden bg-foreground/5 p-2 flex-shrink-0 opacity-80">
              ...
              <p className="text-text-light/70 text-base leading-relaxed mb-8">{project.concept}</p>

              <div>
                <h4 className="text-sm tracking-[0.2em] text-text-light/40 uppercase mb-4">Planned Stack</h4>
                <div className="flex flex-wrap gap-3">
                  {project.plannedTools.map((tool) => (
                    <span key={tool} className="px-4 py-2 text-sm tracking-wider uppercase bg-foreground/5 text-text-light/50 rounded-full border border-foreground/10">
                      {tool}
                    </span>
                  ))}
                </div>
